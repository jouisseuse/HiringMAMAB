...
def generate_initial_prompts_decentralized(candidates, firm_descriptions):
    ...
    for i,candidate in enumerate(candidates):
        ...
        for j,cand in enumerate(candidates):
            if i != j:
                other_candidate += f"C{j+1}: [{cand['features']['category#1']}, {cand['features']['category#2']}, {cand['features']['category#3']}]. "

        candidate_description = (
            f"You are a job candidate C{i+1} with [{candidate['features']['category#1']}, "
            f"{candidate['features']['category#2']}, {candidate['features']['category#3']}] in a hiring game. "
            f"Other candidates have the following features: {other_candidate}"
            "Your goal is to get hired as often as possible by choosing which firm (F1-F60) to apply to each round. "
...
# def generate_initial_prompts(candidates, firm_descriptions):
#     candidate_system_prompts = []
#     firm_system_prompts = []
...
#             "Each round, pick a firm (F1-F60) to apply to and get feedback (hired/not). "
...
def candidate_observation_distributed(a, b, candidate_vector):
    ...
    for j in range(60):  # Outer loop for firms
        firm_result = f"F{j+1}: "
        grouped_features = {}

        for i in range(13):  # Inner loop for features
...
def extract_scores_from_logprobs(choice_logprobs):
    ...
    scores = {str(i): 0 for i in range(1, 61)}
...
async def run_experiment_async_distributed(candidate_file, firm_vector_file, logger):
    ...
    candidate_clients = initialize_clients(60)
    firm_clients = initialize_clients(60)
    ...
    estimate_beta_a_matrix = [np.ones((60, 13), dtype=int) for _ in range(60)]
    estimate_beta_b_matrix = [np.ones((60, 13), dtype=int) for _ in range(60)]
    theta_matrix = np.zeros((4, 13), dtype=float)
    estimate_theta = [theta_matrix.flatten() for _ in range(60)]
    estimate_A = [np.identity(13 * 4) for _ in range(60)]
    estimate_b = [np.zeros(13 * 4) for _ in range(60)]
    all_application_features = {firm: [] for firm in range(60)}
    all_offer_features = {firm: [] for firm in range(60)}
    ...
    for round_num in range(NUM_ROUND):
        ...
        applications = {firm: [] for firm in range(60)}  # initialize applications
        applications_features = {firm: [] for firm in range(60)}  # initialize applications
        candidate_tasks = []
        ...
        for i in range(60):
            ...
            prompt = {
                ...
                "content": (
                    ...
                    "Please choose only one firm (F1-F60) and show the firm ID (e.g., F1) at the end."
        ...
        for i, firm_name in enumerate(response_applications):
            ...
            # firm_index = np.random.randint(0,60)
            ...
            applications_features[firm_index].append(f"C{i+1}:[{selected_candidates['category#1']}, {selected_candidates['category#2']}, {selected_candidates['category#3']}] ")
        ...
        offers = {firm: [] for firm in range(60)} 
        selected_candidates = {firm: [] for firm in range(60)}
        firm_tasks = []
        index_tasks = []
        for i in range(60):
        ...
        for i, candidate_list in selected_candidates.items():
            if candidate_list:
                ...
                # candidate_index = np.random.randint(0,60)
...
def run_experiment_distributed(candidate_file, firm_vector_file, logger, turn, log_dir):
    ...
    # Firm types
    # firm_types = ["technology", "management", "service", "production"]
    firm_types = ["type#1", "type#2", "type#3","type4"]
    # Load firm vectors
    firm_vectors = load_firm_vectors(firm_vector_file)
    # Generate descriptions
    firm_descriptions = generate_firm_descriptions(firm_vectors, firm_types)
    ...
    # Initialize OpenAI clients
    print(f"[DEBUG] Initialize OpenAI clients")
    candidate_clients = initialize_clients_sync(60)
    firm_clients = initialize_clients_sync(60)

    # Initialize result tracking variables
    # Define file paths for saving and restoring states
    state_file = f"result_dir/state60/game_state_distributed_{turn}.json"
    ...
    def initialize_variables():
        ...
        all_application_features = {firm: [] for firm in range(60)}
        all_offer_features = {firm: [] for firm in range(60)}
        candidate_results = {i: [i, 1] for i in range(1, 61)}
        firm_results = {i: [i, 0] for i in range(1, 61)}# (candidate.id, reward)
        
        candidate_messages = [[candidate_system_prompts[i]] for i in range(60)]
        firm_messages = [[firm_system_prompts[i]] for i in range(60)]
    ...
    for round_num in range(current_round, NUM_ROUND):
        ...
        for i in range(60):
            ...
            if len(applications_features[i]) > 0: # skip 0/1
                ...
                firm_messages[i].append(prompt1)
                firm_messages[i].append(prompt2)
                # header = {"round": round_num, "agent": f"F{i+1}"}
                # firm_data = {str(j): 0 for j in range(1, 61)}
                # firm_writer.writerow({**header, **firm_data})
...
def run_experiment_decentralized(candidate_file, firm_vector_file, logger, turn, log_dir):
    ...
    # Initialize result tracking variables
    # Define file paths for saving and restoring states
    state_file = f"result_dir/state60/game_state_decentralized_{turn}.json"
    # Default values for initialization variables
    def initialize_variables():
        ...
        all_offer_features = {firm: [] for firm in range(60)}
        candidate_results = {i: [i, 1] for i in range(1, 61)}
        firm_results = {i: [i, 0] for i in range(1, 61)}
         # (candidate.id, reward)
        candidate_messages = [[candidate_system_prompts[i]] for i in range(60)]
        firm_messages = [[firm_system_prompts[i]] for i in range(60)]
    ...
    for round_num in range(current_round, NUM_ROUND):
        ...
        for i in range(60):
            ...
            if len(applications_features[i]) > 0: # skip 0/1
                ...
                # firm_data = {str(j): 0 for j in range(1, 61)}
                # firm_writer.writerow({**header, **firm_data})
                # firm_writer.writerow({"round": round_num, "agent": f"F{i+1}", "1":0, "2":0,"3":0, "60":0})
...
for turn in range(1, NUM_TURN + 1):
    # Generate a log file name for each iteration
    log_filename = os.path.join(log_dir, f'{turn}.log')

    # Set up a logger specifically for this turn
    logger = logging.getLogger(f'{input_mode}_Run_{turn}')
    ...
    logger.info(f'Starting turn {turn} with mode {input_mode}')
    
    # Run the experiment
    experiment_function('candidate-symbol.json', 'firms_vector.txt',logger, turn, log_dir)

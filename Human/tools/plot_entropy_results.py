...
def process_csv(input_csv):
    ...
    # 1. Overall analysis: cumulative entropy of candidate selections
    overall_entropy = []
    ...
    # Plot cumulative selection bar chart
    ...
    # Create a flag file if any arm exceeds 50% share
    if any(p > 50 for p in arm_percentages):
    ...
    # Color bars: above-average dark, below-average light
    ...
    for player in df.columns:
        ...
        # Plot individual player entropy trend
        plt.figure()
    ...
    # Compute per-round average entropy
    ...
    # 3. Per-player candidate choice scatter
    for player in df.columns:
        player_data = df[player].reset_index()
        player_data.columns = ["Round", "Candidate"]

        # Cast to string, fill missing as "No Choice"

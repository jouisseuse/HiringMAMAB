...
def process_multi_agent_file(csv_file, theta_params, K, T):
    ...
    # 保存所有 agent 的数据
    metrics_list = []
    agents = [f'Candidate{i+1}' for i in range(4)]  # 假设有 4 个 agent: Candidate1, Candidate2, Candidate3, Candidate4
...
def process_plot_for_agents(metrics_df, output_dir):
    agents = [f'Candidate{i+1}' for i in range(4)]  # 假设有 4 个 agent
    for agent in agents:

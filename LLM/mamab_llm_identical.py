"""
LLM-based MAMAB simulation — equal-productivity condition.

Identical to mamab_llm.py except ground_truth="identical" (all arms have
success probability 0.9), so the task has no single objectively best arm.
This condition is used to measure collective bias (entropy of arm selections).
...
def initialize_clients_sync(num_agents: int) -> List[OpenAI]:
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "a local key into DIRECT_API_KEY near the top of mamab_llm_identical.py."
        )

...
def run_contextual_experiment_ref(turn_num, num_rounds, strategies,theta_params, state_filename, logger, csv_filename):
    # Try to load the state if it exists, otherwise start from scratch
    try:
        llm_prompt, chosen_arms, llm_choices, strategies, current_round, reward_history = load_state(state_filename)
        logger.info("Resuming from saved state")
...
TEST_TYPE = f"{SCENARIO}_{FEATURES_TYPES}_{MODEL_NAME}_{TEMPERATURE}_REF_without"
...
state_dir = f'result_dir/state_hiring'

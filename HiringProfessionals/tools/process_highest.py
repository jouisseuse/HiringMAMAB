...
def load_experiment_data(folder_path):
    """Load all playerRound_processed.csv files from folder and return as a list of DataFrames."""
...
def calculate_optimal_arm_rate(data, condition):
    """Compute per-round optimal arm selection rate and 95% CI across experiments."""
...
def analyze_statistical_significance(df_comm, df_no_comm):
    """Run OLS regression to test condition effect: b, 95% CI, p-value."""
...
def plot_optimal_arm_rate(df_comm, df_no_comm, save_path):
    """Plot optimal arm selection rate over rounds and save to PDF."""

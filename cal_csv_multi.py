...
def calculate_agent_metrics(df, agent_name, theta_params, K, T):
    ...
    for index, row in df.iterrows():
        ...
        highest_estimated_arm = estimated_rewards.index(max(estimated_rewards)) + 1  
...
def process_multi_agent_file(csv_file, theta_params, K, T):
    ...
    # 保存所有 agent 的数据
    metrics_list = []
    agents = [f'Agent{i+1}' for i in range(4)]  # 假设有 4 个 agent: Agent1, Agent2, Agent3, Agent4
...
def process_plot_for_agents(metrics_df, output_dir):
    agents = [f'Agent{i+1}' for i in range(4)]  # 假设有 4 个 agent
    for agent in agents:

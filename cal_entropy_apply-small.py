...
def process_directory(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.log'):
            ...
            for line in data:
                match = re.search(r'Applicants entropy all:\s*([\d\.]+)', line)
                if match:
...
csv_output1 = os.path.join(directory1, "application_results.csv")
csv_output2 = os.path.join(directory2, "application_results.csv")
...
max_gender_entropy = stats.entropy([0.25]* 4, base=2)
total_max_entropy = max_gender_entropy
...
plt.title('Application Entropy Trend with 95% Confidence Interval and Baseline')
plt.xlabel('Round')
...
plot_output = os.path.join(directory1, "entropy_trend_apply.png")
